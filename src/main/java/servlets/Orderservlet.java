...
public class Orderservlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        JSONArray cartItems = new JSONArray(cart);
        List<Order> orders = new ArrayList<>();
        log("From Order Servlet");
        log(email);
        log(name);
        log(address);
        log(number);
        ...
        for(int i = 0;i<cartItems.length();i++){
            ...
            int productQuantity = cartItem.getInt("productQuantity");
            double productPrice = cartItem.getDouble("productPrice");
